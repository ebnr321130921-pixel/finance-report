...
def split_period(v):
    ...
    if len(parts) == 2:
        return f"{parts[0]}<br><span class='soft'>{parts[1]}</span>"
    return str(v)
...
def build_histogram_svg(series, title, mean=None, s1=None, s3=None, latest=None):
    ...
    <text x="10" y="{bottom:.1f}"
      ...
      text-anchor="start"
      transform="rotate(-90 10 {bottom:.1f})">{title}</text>
...
if FORECAST_TODAY_PATH.exists():
    ...
    for asset in ["QQQ", "SP500"]:
        ...
        for h in ["1W", "1M", "6M"]:
            matched = sub[sub["forecast_horizon"].astype(str).str.upper() == h]
            if matched.empty:
                continue
            r = matched.iloc[0]
            ...
            <tr>
              <td>{asset}</td>
              <td class="soft">{h}</td>
              <td class="soft">{split_period(r["forecast_period"])}</td>
...
if DECISION_CUR_PATH.exists():
    ...
    for _, r in ddf.iterrows():
        ...
        <tr>
          ...
          <td class="num {shock_color(r['shock_rank'])}">
            {float(r['shock_score']):.2f}
          </td>
          <td class="{shock_color(r['shock_rank'])}">
            {r['shock_rank']}
          </td>
        </tr>
        """)
...
SIGMA_PATH = DATA_DIR / "aegis_sigma_diagnostics.csv"
...
if TREND_PATH.exists():
    ...
    start  = center - pd.DateOffset(months=12)
    end    = center + pd.DateOffset(months=6)
    ...
    def trend_col(name):
        if name in tdf.columns:
            return tdf[name]
        return pd.Series([pd.NA] * len(tdf))
    ...
    if decision_cdf is not None:
        ...
        # --- P95 / P99 from aegis_sigma_diagnostics (week aligned) ---
        if sigma_df is not None:
            sigma_df["week"] = pd.to_datetime(sigma_df["week"])
            sigma_sync = (
                sigma_df
                .set_index("week")
                .reindex(base_weeks)
        ...
        <canvas id="decWeekly" height="260"></canvas>
        <h3 class="soft">Monthly</h3>
        <canvas id="decMonthly" height="260"></canvas>
        ...
        function niceScaleFromDatasets(datasets, floorZero=true, targetTicks=6) {
          const values = datasets
            .flatMap(ds => ds.data || [])
            .filter(v => v !== null && !isNaN(v));

          if (values.length === 0) return { min: 0, max: 1, stepSize: 0.2 };
        ...
        function buildDecision(id, datasets) {
          const yScale = niceScaleFromDatasets(datasets, true, 6);
          new Chart(
              ...
              scales: {
                y: {
                  ...
                  x: {
                    ...
                    ticks: { minRotation: 90, maxRotation: 90, autoSkip: true, maxTicksLimit: 18 }
                  }
                },
                plugins: {
                  legend: {
                    position: 'bottom',
                    labels: {
                      filter: item => !item.text.includes('P95') && !item.text.includes('P99')
                    }
                  }
                }
              }
        ...
        empty_threshold = pd.Series([pd.NA] * len(base_weeks))

        def threshold_array(col):
            if sigma_sync is None or col not in sigma_sync.columns:
                return js_arr(empty_threshold)
            return js_arr(sigma_sync[col])
        ...
        decision_chart_html = (
            ...
            # --- Weekly thresholds ---
            .replace("__QQQ_1W_P95__", threshold_array("qqq_1w_p95"))
            .replace("__QQQ_1W_P99__", threshold_array("qqq_1w_p99"))
            .replace("__SP_1W_P95__",  threshold_array("sp_1w_p95"))
            .replace("__SP_1W_P99__",  threshold_array("sp_1w_p99"))

            # --- Monthly thresholds ---
            .replace("__QQQ_1M_P95__", threshold_array("qqq_1m_p95"))
            .replace("__QQQ_1M_P99__", threshold_array("qqq_1m_p99"))
            .replace("__SP_1M_P95__",  threshold_array("sp_1m_p95"))
            .replace("__SP_1M_P99__",  threshold_array("sp_1m_p99"))
        )
    ...
    <script>
        ...
        function calcSharedY(seriesList) {{
          const values = seriesList
            .flatMap(series => series || [])
            .filter(v => v !== null && !isNaN(v));

          if (values.length === 0) {{
            return {{ min: -1, max: 1, stepSize: 0.5 }};
          }}
        ...
        const sharedY = {{
          ...
          ticks: {{
            stepSize: forecastY.stepSize,
            callback: v => v + "%"
          }}
        }};


        function buildTrend(id, pred1m, act1m) {{
          new Chart(
            document.getElementById(id),
            {{
              type: 'line',
              data: {{
                labels: labels,
                datasets: [
                  {{ label: 'Forecast (1M)', data: pred1m, borderDash: [3,3], borderWidth: 1 }},
                  {{ label: 'Actual (1M)',   data: act1m,  borderWidth: 1 }}
                ]
              }},
              options: {{
                responsive: true,
