...
if FORECAST_TODAY_PATH.exists():
    ...
    def find_col_df(df, candidates):
        for c in df.columns:
            cl = c.lower()
            for k in candidates:
                if k.lower() in cl:
                    return c
        raise KeyError(f"Column not found: {candidates} in {list(df.columns)}")

    A = find_col_df(fdf, ["asset", "target", "symbol", "index"])
    H = find_col_df(fdf, ["forecast_horizon", "horizon"])
    P = find_col_df(fdf, ["forecast_period", "period"])
    R = find_col_df(fdf, ["forecast_return", "pred", "prediction"])
    D = find_col_df(fdf, ["actual_direction", "forecast_direction", "direction"])

    def period_2lines(v: str) -> str:
        if "→" in str(v):
            s, e = map(str.strip, v.split("→", 1))
            return f"{s}<br><span class='gray'>{e}</span>"
        return str(v)
    ...
    for asset in ["QQQ", "SP500"]:
        ...
        for h in ["1W", "1M"]:
            r = sub[sub[H].astype(str).str.upper() == h].iloc[0]
            rows.append(f"""
            <tr>
              <td><b>{asset}</b></td>
              <td class="gray"><b>{h}</b></td>
              <td><b>{float(r[R])*100:.2f}%</b></td>
              <td class="dir-{str(r[D]).lower()}"><b>{r[D]}</b></td>
              <td class="gray">{period_2lines(r[P])}</td>
            </tr>
            """)
...
.dir-up {{ color:#34c759; }}
.dir-down {{ color:#ff3b30; }}
.dir-neutral {{ color:#8e8e93; }}
...
<!-- Market Direction Log (Prediction)
     forward_direction_forecast は現行 Viewer では未使用のため非表示 -->
