...
# =========================================================
# Indicator Notes (from CSV)  ※ OPTIONAL
# =========================================================
NOTES_PATH = DATA_DIR / "market_indicator_notes.csv"
    ...
    for asset in ["QQQ", "SP500"]:
        ...
        for _, r in sub.iterrows():
            ...
            <tr>
              ...
              <td class="soft">{period_2lines(r["forecast_period"])}</td>
              <td class="num">{pred:.2f}%</td>
              <td class="num">{act:.2f}%</td>
            </tr>
            """)
...
# =========================================================
# Forecast Trend Chart (Weekly / Display Only)
# =========================================================
TREND_PATH = DATA_DIR / "forecast_trend_weekly.csv"

forecast_trend_html = ""
if TREND_PATH.exists():
    tdf = pd.read_csv(TREND_PATH, parse_dates=["week"])
    tdf = tdf.sort_values("week")

    def js_array(col):
        return "[" + ",".join("null" if pd.isna(v) else f"{v:.6f}" for v in tdf[col]) + "]"

    labels = "[" + ",".join(f"'{d.strftime('%Y-%m-%d')}'" for d in tdf["week"]) + "]"

    forecast_trend_html = f"""
    <div class="card">
    <details>
    <summary>Forecast Trend (1M only)</summary>

    <h3 style="margin:10px 0 4px;">QQQ</h3>
    <canvas id="trendQQQ" height="180"></canvas>

    <h3 style="margin:14px 0 4px;">SP500</h3>
    ...
    new Chart(document.getElementById('trendQQQ'), {{
      type:'line',
      data:{{
        labels:labels,
        datasets:[
          ...
          {{
            label:'Actual (1M)',
            data:{js_array("qqq_actual_1m_cum")},
            borderWidth:1,
            fill:false
          }}
        ]
    ...
    new Chart(document.getElementById('trendSP'), {{
      type:'line',
      data:{{
        labels:labels,
        datasets:[
          ...
          {{
            label:'Actual (1M)',
            data:{js_array("sp_actual_1m_cum")},
            borderWidth:1,
            fill:false
          }}
        ]
...
<div class="card soft">
  RISK<br>
  <span class="num status-risk">{risk_label}</span>
</div>
...
f"<td class='{('status-safe' if str(s).upper() in ['HIGH','UP'] else 'status-risk' if str(s).upper() in ['LOW','DOWN'] else 'soft')}'>{s}</td>"
...
<div class="card">
<details>
<summary>Indicator Notes</summary>
{indicator_notes_html}
</details>
</div>
