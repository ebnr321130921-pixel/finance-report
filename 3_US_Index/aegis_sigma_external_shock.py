...
for asset in ["qqq", "sp"]:
    for hz in ["1w", "1m"]:
        ...
        # --- rolling z-score on cumulative error ---
        # 累積誤差そのものを sigma で割ると、長期の水準差で score が肥大化する。
        # rolling 平均からの乖離に変換し、直近分布に対するショック度として扱う。
        err_mean_26 = err.rolling(26, min_periods=26).mean()
        sigma_26 = err.rolling(26, min_periods=26).std()
        shock_score = (err - err_mean_26).abs() / sigma_26.replace(0, pd.NA)
...
for _, r in df.iterrows():
    ...
    for asset in ASSETS:
        for hz in HORIZONS:
            ...
            elif score >= p99_val or score >= 3.0:
                ext_flag = True
                rank_adj = "EXTREME (P99)"
            elif score >= p95_val or score >= 2.0:
                ext_flag = False
                rank_adj = "ELEVATED (P95)"
            else:
                ext_flag = False
                rank_adj = "NORMAL RANGE"
