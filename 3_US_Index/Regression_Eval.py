...
- RELATIVE はログ用途のみ（Viewer / 判断には使用しない）
...
if "date" not in raw_df.columns:
    raise ValueError("market_factors_raw.csv must have 'date' column")
...
log_rows.append({
    ...
    "target": "RELATIVE",
    "horizon": "1W",
    "start_date": pred_1w_start,
    "end_date": pred_1w_end,

    "pred_sum": (
        results["QQQ_1W_FWD_SUM"]["point_prediction"]
        - results["SP500_1W_FWD_SUM"]["point_prediction"]
    ),
...
log_rows.append({
    ...
    "target": "RELATIVE",
    "horizon": "1M",
    "start_date": pred_1m_start,
    "end_date": pred_1m_end,

    "pred_sum": (
        results["QQQ_1M_FWD_SUM"]["point_prediction"]
        - results["SP500_1M_FWD_SUM"]["point_prediction"]
    ),
...
# --- RELATIVE actual ---
for h in ["1W", "1M"]:
    mask_rel = (merged["target"] == "RELATIVE") & (merged["horizon"] == h)
    mask_q   = (merged["target"] == "QQQ") & (merged["horizon"] == h)
    mask_sp  = (merged["target"] == "SP500") & (merged["horizon"] == h)

    merged.loc[mask_rel, "actual_sum"] = (
        merged.loc[mask_q, "actual_sum"].values
        - merged.loc[mask_sp, "actual_sum"].values
    )
...
# =========================================================
# TODAY MARKET DECISION (APP VIEW)  ※ CLEAN & FINAL
# =========================================================
...
# NOTE:
# RELATIVE は検証・分析用途のみ。
# Viewer（意思決定UI）では QQQ / SP500 の絶対予測のみを表示する。
today_df = merged[
...
# ---- 表示用変換 ----
today_df["forecast_direction"] = today_df["pred_sum"].apply(
    lambda x: "UP" if x > 0 else ("DOWN" if x < 0 else "FLAT")
)

today_df["actual_direction"] = today_df["actual_sum"].apply(
    lambda x: (
        "UP" if x > 0 else ("DOWN" if x < 0 else "FLAT")
    ) if pd.notna(x) else ""
)

today_df["direction_match"] = np.where(
    today_df["actual_sum"].notna(),
    today_df["forecast_direction"] == today_df["actual_direction"],
    np.nan
)

# Viewer 表示専用期間表現（LOG には保存しない）
today_df["period"] = (
...
today_df_final = (
    ...
    .rename(columns={
        ...
        "forecast_direction": "predicted_direction",
...
print(f"=== TODAY MARKET DECISION SAVED : {TODAY_PATH.name} ===")
