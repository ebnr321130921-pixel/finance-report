...
required_cols = ["MRDI_Short", "MRDI_Long", "RiskScore"]

if all(c in md_df.columns for c in required_cols):

    scatter = md_df[["date", "MRDI_Short", "MRDI_Long", "RiskScore"]].copy()
    scatter.columns = ["date", "mrdi_short", "mrdi_long", "risk_score"]
    ...
    for col in ["mrdi_short", "mrdi_long", "risk_score"]:
        s = scatter[col]
        mean   = s.mean()
        std    = s.std()
        median = s.median()
