...
HOLDINGS_INPUT_CSV = BASE / "holdings_input.csv"
...
def product_url_map():
    return {p.display_name: p.source_url for p in load_products()}
...
def load_holdings():
    ...
    if raw:
        data = json.loads(raw)
    elif HOLDINGS_INPUT_CSV.exists():
        return pd.read_csv(HOLDINGS_INPUT_CSV, encoding="utf-8-sig")
...
def auto_activate_planned_holdings():
    if not HOLDINGS_INPUT_CSV.exists() or not RECORDS.exists():
        return

    holdings = pd.read_csv(HOLDINGS_INPUT_CSV, encoding="utf-8-sig")
    if holdings.empty or "status" not in holdings.columns:
        return

    records = normalize_records(load_records())
    if records.empty or "global_market_date" not in records.columns:
        return

    records["global_market_date"] = pd.to_datetime(records["global_market_date"], errors="coerce")
    latest = records["global_market_date"].max()
    if pd.isna(latest):
        return
    ...
    for idx, row in holdings.iterrows():
        ...
        planned_value = pd.to_numeric(row.get("planned_value"), errors="coerce")
        if status != "planned" or not pd.isna(units) and units > 0 or pd.isna(planned_value) or planned_value <= 0:
            continue

        trade_date = pd.to_datetime(row.get("change_date"), errors="coerce")
        if pd.isna(trade_date):
            trade_date = pd.to_datetime(row.get("Zero_date"), errors="coerce")
        if pd.isna(trade_date) or trade_date > latest:
            continue

        product = str(row.get("product", "")).strip()
        if product not in records.columns:
            print(f"[WARN] planned holding product has no NAV column: {product}")
            continue

        available = records[
            (records["global_market_date"] >= trade_date) &
            (records[product].notna())
        ].sort_values("global_market_date")
        if available.empty:
            continue

        nav = pd.to_numeric(available.iloc[0][product], errors="coerce")
        if pd.isna(nav) or nav <= 0:
            continue

        holdings.at[idx, "units"] = int(round(planned_value * 10000 / nav))
        holdings.at[idx, "status"] = "active"
        holdings.at[idx, "change_date"] = available.iloc[0]["global_market_date"].strftime("%Y/%m/%d")
        holdings.at[idx, "Zero_date"] = available.iloc[0]["global_market_date"].strftime("%Y/%m/%d")
        changed = True
        print(
            f"Auto-activated planned holding: {product} "
            f"units={holdings.at[idx, 'units']} nav={int(nav)}"
        )

    if changed:
        holdings.to_csv(HOLDINGS_INPUT_CSV, index=False, encoding="utf-8-sig")
...
def ensure_product_columns(rec, product):
    s = product.short
    market_date_col = f"{s}_market_date"
    if market_date_col not in rec.columns:
        # Market dates are normalized strings.  Initializing this column with
        # np.nan makes pandas 3 infer float64 and reject later string writes.
        rec[market_date_col] = pd.Series(pd.NA, index=rec.index, dtype="object")
...
def backfill_product_history(product):
    ...
    rec = calc_prev_and_cum(rec)
    save_records(rec)
    print(f"Backfilled {product.short}: history={len(history)} added_dates={added_count}")


def backfill_histories(targets, continue_on_error=False):
    ...
    for product in products:
        try:
            ...
            if not continue_on_error:
                raise
            failures.append((product.short, exc))
            print(f"[WARN] Backfill skipped for {product.short}: {exc}")

    if failures:
        failed_names = ", ".join(short for short, _ in failures)
        print(
            "[WARN] Historical NAV sync was incomplete, but the latest-price "
            f"update will continue: {failed_names}"
        )
...
def load_source():
    ...
    holding_products = holdings["product"].astype(str).str.strip()
    holdings["product"] = holding_products.map(PRODUCT_MAP).fillna(holding_products)
...
def build_summary_portfolio_pnl(records, holdings):
    ...
    if "settlement_date" not in holdings.columns:
        holdings["settlement_date"] = pd.NaT
    holdings["settlement_date"] = pd.to_datetime(holdings["settlement_date"], errors="coerce")
    holdings["change_value"] = pd.to_numeric(holdings["change_value"], errors="coerce")
    holdings["units"] = pd.to_numeric(holdings["units"], errors="coerce").fillna(0)
    if "planned_value" not in holdings.columns:
        holdings["planned_value"] = np.nan
    holdings["planned_value"] = pd.to_numeric(holdings["planned_value"], errors="coerce").fillna(0)


    # 通常商品行のみ
    holdings_prod = holdings[holdings["product"].notna()]

    df = holdings_prod.merge(today, on="product", how="left")
    df = df[(df["units"] > 0) | (df["planned_value"] > 0)]

    # 現在評価額
    df["value"] = np.where(
        df["units"] > 0,
        df["units"] * df["nav"] / 10000,
        df["planned_value"]
    )

    # 今日の損益（円）
    df["daily_pnl_yen"] = np.where(
        df["units"] > 0,
        df["units"] * df["nav"] * df["pct"] / 100 / 10000,
        np.nan
    )
    ...
    df["trade_date"] = df["change_date"].fillna(df["Zero_date"])
    ...
    out = df.assign(
        ...
        "trade_date","settlement_date","base_nav","base_value",
        "planned_value",
        "value",
...
# ------------------------------------------------------------
# Account return (start_value -> current, CAGR)
# ------------------------------------------------------------

def build_account_return(records, holdings, account_name, output_filename=None):
    latest = records["market_date"].max()

    account = holdings.get("account", holdings.get("status", "")).astype(str).str.lower()
    target = holdings[account == account_name.lower()].copy()
    if target.empty:
        return None

    target["units"] = pd.to_numeric(target["units"], errors="coerce").fillna(0)
    if "planned_value" not in target.columns:
        target["planned_value"] = np.nan
    target["planned_value"] = pd.to_numeric(target["planned_value"], errors="coerce").fillna(0)
    target = target[(target["units"] > 0) | (target["planned_value"] > 0)]
    if target.empty:
        return None

    target["start"] = pd.to_datetime(target["start"], errors="coerce")
    target["Zero_date"] = pd.to_datetime(target["Zero_date"], errors="coerce")
    target["change_date"] = pd.to_datetime(target["change_date"], errors="coerce")

    target["start_value"] = pd.to_numeric(
        target["start_value"].astype(str).str.replace(",", "", regex=False),
        errors="coerce"
    )
    target["change_value"] = pd.to_numeric(
        target["change_value"].astype(str).str.replace(",", "", regex=False),
        errors="coerce"
    )

    target["basis_date"] = target["start"]
    target["basis_date"] = target["basis_date"].fillna(target["Zero_date"])
    target["basis_date"] = target["basis_date"].fillna(target["change_date"])
    start_date = target["basis_date"].min()

    if pd.isna(start_date):
        print(f"[WARN] {account_name} start_date is invalid")
        return None
    ...
    target["nav"] = target["product"].map(nav_map)
    target["current_value"] = np.where(
        target["units"] > 0,
        target["units"] * target["nav"] / 10000,
        target["planned_value"]
    )
    end_value = target["current_value"].sum()

    nav_by_product = records.sort_values("market_date")

    def lookup_basis_nav(row):
        if pd.isna(row["basis_date"]):
            return np.nan

        available = nav_by_product[
            (nav_by_product["product"] == row["product"]) &
            (nav_by_product["market_date"] >= row["basis_date"]) &
            (nav_by_product["nav"].notna())
        ]
        return available.iloc[0]["nav"] if not available.empty else np.nan

    target["basis_nav"] = target.apply(lookup_basis_nav, axis=1)
    target["basis_value"] = target["start_value"]
    target["basis_value"] = target["basis_value"].fillna(target["change_value"])
    target["basis_value"] = target["basis_value"].fillna(target["planned_value"].where(target["planned_value"] > 0))
    target["basis_value"] = target["basis_value"].fillna(target["units"] * target["basis_nav"] / 10000)

    start_value = target["basis_value"].sum()
    if pd.isna(start_value) or start_value <= 0 or pd.isna(end_value):
        print(f"[WARN] {account_name} start_value or end_value is invalid")
        return None
    ...
    if months > 0:
        ...
        xirr_rate = xirr(cashflows) if len(cashflows) >= 2 else np.nan

    # ======================================================
    # 出力
    # ======================================================
    years_label = round(years, 2) if years > 0 else 0
    out = pd.DataFrame([{
        "type": account_name,
        "start_date": start_date.strftime("%Y-%m-%d"),
        "end_date": max(latest, start_date).strftime("%Y-%m-%d"),
        "years": years_label,
        "months": max(months, 0),
        ...
        "xirr_monthly_cf_pct": round(xirr_rate * 100, 2) if not pd.isna(xirr_rate) else np.nan
    }])

    if output_filename:
        write_output(out, output_filename)

    return out


def build_ideco_return(records, holdings):
    return build_account_return(records, holdings, "iDeCo", "summary_ideco_return.csv")


def build_nisa_return(records, holdings):
    return build_account_return(records, holdings, "NISA", "summary_nisa_return.csv")


def build_taxable_return(records, holdings):
    return build_account_return(records, holdings, "特定", "summary_taxable_return.csv")


def build_account_monthly_trends(records, holdings):
    records = records.copy().sort_values("market_date")
    holdings = holdings.copy()
    holdings["units"] = pd.to_numeric(holdings["units"], errors="coerce").fillna(0)
    if "planned_value" not in holdings.columns:
        holdings["planned_value"] = np.nan
    holdings["planned_value"] = pd.to_numeric(holdings["planned_value"], errors="coerce").fillna(0)
    holdings["change_value"] = pd.to_numeric(holdings["change_value"], errors="coerce")
    holdings["start_value"] = pd.to_numeric(holdings["start_value"], errors="coerce")
    holdings["start"] = pd.to_datetime(holdings["start"], errors="coerce")
    holdings["Zero_date"] = pd.to_datetime(holdings["Zero_date"], errors="coerce")
    holdings["change_date"] = pd.to_datetime(holdings["change_date"], errors="coerce")
    holdings["basis_date"] = holdings["start"].fillna(holdings["Zero_date"]).fillna(holdings["change_date"])

    latest_month = records["market_date"].dt.to_period("M").max()
    chart_months = pd.period_range(end=latest_month, periods=12, freq="M")
    months = pd.period_range(end=latest_month, periods=13, freq="M")
    nav = records.groupby([records["market_date"].dt.to_period("M"), "product"])["nav"].last().unstack("product")
    nav = nav.reindex(months)
    ...
    holdings["basis_nav"] = holdings.apply(lookup_basis_nav, axis=1)
    holdings["basis_value"] = holdings["start_value"]
    holdings["basis_value"] = holdings["basis_value"].fillna(holdings["change_value"])
    holdings["basis_value"] = holdings["basis_value"].fillna(holdings["planned_value"].where(holdings["planned_value"] > 0))
    holdings["basis_value"] = holdings["basis_value"].fillna(holdings["units"] * holdings["basis_nav"] / 10000)

    out = {}
    for account in ["iDeCo", "NISA", "特定"]:
        lots = holdings[
            (holdings["account"].astype(str).str.lower() == account.lower()) &
            ((holdings["units"] > 0) | (holdings["planned_value"] > 0))
        ].copy()
        if lots.empty:
            out[account] = pd.DataFrame(columns=["month", "value", "gain_pct"])
            continue
        ...
        for month in months:
            ...
            for _, lot in lots.iterrows():
                basis_date = lot["basis_date"]
                if pd.isna(basis_date):
                    continue
                if month < basis_date.to_period("M"):
                    continue

                base_value = lot["basis_value"]
                if pd.isna(base_value) or base_value <= 0:
                    continue

                units = lot["units"]
                planned_value = lot["planned_value"]
                if units > 0:
                    product_nav = nav.loc[month, lot["product"]] if lot["product"] in nav.columns else np.nan
                    if pd.isna(product_nav):
                        continue
                    lot_value = units * product_nav / 10000
                elif planned_value > 0:
                    lot_value = planned_value
                else:
                    continue

                month_value += lot_value
                month_base += base_value

            cumulative_gain_yen = month_value - month_base if month_base else np.nan
            cumulative_gain_pct = cumulative_gain_yen / month_base * 100 if month_base else np.nan
            rows.append({
                "month": month.strftime("%Y/%m"),
                "value": month_value if month_value else np.nan,
                "base_value": month_base if month_base else np.nan,
                "cumulative_gain_yen": cumulative_gain_yen,
                "cumulative_gain_pct": cumulative_gain_pct,
            })
        trend = pd.DataFrame(rows)
        trend["monthly_gain_yen"] = trend["cumulative_gain_yen"].diff()
        first_valid = trend["cumulative_gain_yen"].first_valid_index()
        if first_valid is not None and pd.isna(trend.loc[first_valid, "monthly_gain_yen"]):
            trend.loc[first_valid, "monthly_gain_yen"] = trend.loc[first_valid, "cumulative_gain_yen"]
        trend["monthly_gain_pct"] = np.where(
            trend["base_value"].notna() & (trend["base_value"] > 0),
            trend["monthly_gain_yen"] / trend["base_value"] * 100,
            np.nan,
        )
        trend["_period"] = pd.PeriodIndex(trend["month"], freq="M")
        trend = trend[trend["_period"].isin(chart_months)].drop(columns=["_period"]).reset_index(drop=True)
        out[account] = trend
...
# ============================================================
def load_data():
    auto_activate_planned_holdings()
    records, holdings = load_source()
    ideco = build_ideco_return(records, holdings)
    if ideco is None:
        ideco = pd.DataFrame()
    nisa = build_nisa_return(records, holdings)
    if nisa is None:
        nisa = pd.DataFrame()
    taxable = build_taxable_return(records, holdings)
    if taxable is None:
        taxable = pd.DataFrame()
    account_monthly = build_account_monthly_trends(records, holdings)
...
def build_html(
    ...
    weekly, cum_weekly,
    monthly, cum_monthly,
    ideco, nisa, taxable, account_monthly
):
    market = market[market["product"].isin(VIEWER_PRODUCTS)].copy()
    portfolio = portfolio[
        portfolio["product"].isin(VIEWER_PRODUCTS) |
        portfolio["account"].isin(["現金"])
    ].copy()
    chart_columns = (
        set(daily.columns)
        & set(cum_daily.columns)
        & set(weekly.columns)
        & set(cum_weekly.columns)
        & set(monthly.columns)
        & set(cum_monthly.columns)
    )
    products = [p for p in VIEWER_PRODUCTS if p in chart_columns]
    ...
    AXIS_LINE = (
        ...
        "interaction:{mode:'index',intersect:false},"
        "layout:{padding:{top:18,bottom:12,left:6,right:10}},"
        "elements:{line:{tension:0,borderWidth:3},point:{radius:0,hoverRadius:5,hitRadius:12}},"
        "plugins:{"
    ...
    <style>
            ...
            * { box-sizing:border-box; }
            ...
            a.product-link:hover {
                ...
                border-bottom-color:var(--accent-pos);
            }
...
<tr><th>Product</th><th>Date</th><th>NAV</th><th>Daily %</th><th>Δ ¥ /10k</th></tr>"""
    for _, r in market.iterrows():
        ...
        url = URL_MAP.get(product, "")
        product_label = (
            f"<a class='product-link' href='{url}' target='_blank' rel='noopener noreferrer'>{product}</a>"
            if url else product
        )

        html += (
            f"<tr><td>{product_label}</td>"
            f"<td>{r['date']}</td>"
    ...
    # ========================================================
    portfolio_lots = portfolio.copy()
    total_value = portfolio_lots["value"].sum()
    total_base_value = portfolio_lots["base_value"].sum()
    total_daily_pnl = portfolio_lots["daily_pnl_yen"].fillna(0).sum()
    total_since_pnl = portfolio_lots["since_change_pnl_yen"].sum()
    total_since_pct = total_since_pnl / total_base_value * 100 if total_base_value else np.nan
    personal_accounts = ["NISA", "特定", "現金"]
    personal_lots = portfolio_lots[portfolio_lots["account"].isin(personal_accounts)]
    active_personal_accounts = [
        account for account in personal_accounts
        if account in set(personal_lots["account"].dropna())
    ]
    personal_label = "Personal"
    if active_personal_accounts:
        personal_label += f" ({' + '.join(active_personal_accounts)})"
    personal_value = personal_lots["value"].sum()
    personal_base_value = personal_lots["base_value"].sum()
    personal_daily_pnl = personal_lots["daily_pnl_yen"].fillna(0).sum()
    personal_since_pnl = personal_lots["since_change_pnl_yen"].sum()
    personal_since_pct = personal_since_pnl / personal_base_value * 100 if personal_base_value else np.nan

    html += """<details class="private-section"><summary><span class="section-title">Portfolio (Private)</span></summary>
<div class="card"><h2 class="section-title">Total Assets</h2>
<table class="portfolio-table">
<tr><th>Account</th><th>Value ¥</th><th>Daily ¥</th><th>Since ¥</th><th>Since %</th></tr>"""
    html += (
        f"<tr class='total-row'><td>All Assets</td>"
        f"<td>{fmt_yen(total_value, show_plus=False)}</td>"
        f"<td class='{sign_class(total_daily_pnl)}'>{fmt_yen(total_daily_pnl)}</td>"
        f"<td class='{sign_class(total_since_pnl)}'>{fmt_yen(total_since_pnl)}</td>"
        f"<td class='{sign_class(total_since_pct)}'>{fmt_pct(total_since_pct)}</td></tr>"
    )
    html += (
        f"<tr class='total-row'><td>{personal_label}</td>"
        f"<td>{fmt_yen(personal_value, show_plus=False)}</td>"
        f"<td class='{sign_class(personal_daily_pnl)}'>{fmt_yen(personal_daily_pnl)}</td>"
        f"<td class='{sign_class(personal_since_pnl)}'>{fmt_yen(personal_since_pnl)}</td>"
        f"<td class='{sign_class(personal_since_pct)}'>{fmt_pct(personal_since_pct)}</td></tr>"
    )
    for account, rows in portfolio_lots.groupby("account", sort=False):
        account_value = rows["value"].sum()
        account_base_value = rows["base_value"].sum()
        account_daily_pnl = rows["daily_pnl_yen"].fillna(0).sum()
        account_since_pnl = rows["since_change_pnl_yen"].sum()
        account_since_pct = account_since_pnl / account_base_value * 100 if account_base_value else np.nan
        html += (
            f"<tr><td>{account}</td>"
            f"<td>{fmt_yen(account_value, show_plus=False)}</td>"
            f"<td class='{sign_class(account_daily_pnl)}'>{fmt_yen(account_daily_pnl)}</td>"
            f"<td class='{sign_class(account_since_pnl)}'>{fmt_yen(account_since_pnl)}</td>"
            f"<td class='{sign_class(account_since_pct)}'>{fmt_pct(account_since_pct)}</td></tr>"
        )
    html += """</table></div>
<div class="card"><h2 class="section-title">Portfolio</h2>
<table class="portfolio-table">
<tr><th>Product</th><th>Account</th><th>Status</th><th>Units</th><th>Value ¥</th><th>Daily ¥</th><th>Since ¥</th><th>Since %</th></tr>"""

    grouped_rows = []
    for (product, account), lots in portfolio_lots.groupby(["product", "account"], sort=False):
        value = lots["value"].sum()
        base_value = lots["base_value"].sum()
        daily_pnl_yen = lots["daily_pnl_yen"].fillna(0).sum()
        since_change_pnl_yen = lots["since_change_pnl_yen"].sum()
        statuses = [str(s) for s in lots["status"].dropna().unique()]
        status = statuses[0] if len(statuses) == 1 else "mixed"
        grouped_rows.append({
            "product": product,
            "account": account,
            "status": status,
            "units": lots["units"].sum(),
            "value": value,
            "daily_pnl_yen": daily_pnl_yen,
            "daily_pnl_pct": daily_pnl_yen / (value - daily_pnl_yen) * 100 if value != daily_pnl_yen else np.nan,
            "since_change_pnl_yen": since_change_pnl_yen,
            "since_change_pnl_pct": since_change_pnl_yen / base_value * 100 if base_value else np.nan,
            "lots": lots.sort_values(["trade_date", "settlement_date"], na_position="last"),
        })
    ...
    for i, r in enumerate(grouped_rows):
        ...
        units = "" if r["units"] <= 0 else int(r["units"])
        detail_id = f"lot-detail-{i}"
        lot_rows = """
        <table class="lot-table">
        <tr><th>Trade</th><th>Status</th><th>Units</th><th>Investment</th><th>Value ¥</th><th>Gain</th><th>Gain %</th></tr>"""
        for _, lot in r["lots"].iterrows():
            lot_units = "" if lot["units"] <= 0 else int(lot["units"])
            lot_cls_y = sign_class(lot.get("since_change_pnl_yen"))
            lot_cls_p = sign_class(lot.get("since_change_pnl_pct"))
            lot_rows += (
                f"<tr><td>{fmt_date(lot.get('trade_date'))}</td>"
                f"<td>{lot.get('status', '')}</td>"
                f"<td>{lot_units}</td>"
                f"<td>{fmt_yen(lot.get('base_value'), show_plus=False)} ¥</td>"
                f"<td>{fmt_yen(lot.get('value'), show_plus=False)}</td>"
                f"<td class='{lot_cls_y}'>{fmt_yen(lot.get('since_change_pnl_yen'))} ¥</td>"
                f"<td class='{lot_cls_p}'>{fmt_pct(lot.get('since_change_pnl_pct'))}</td></tr>"
            )
        lot_rows += "</table>"

        html += (
            f"<tr class='portfolio-main' onclick=\"toggleLotRow('{detail_id}')\"><td>{r['product']}</td>"
            ...
            f"<tr id='{detail_id}' class='lot-detail-row'><td class='lot-detail-cell' colspan='8'>{lot_rows}</td></tr>"
        )
    html += "</table></div>"

    def nice_axis_max(values, step):
        vals = [float(v) for v in values if not pd.isna(v)]
        if not vals:
            return step
        return int(np.ceil(max(vals) / step) * step)

    def nice_pct_bounds(values):
        vals = [float(v) for v in values if not pd.isna(v)]
        if not vals:
            return -10, 10
        mn = min(vals)
        mx = max(vals)
        low = min(0, np.floor(mn / 10) * 10)
        high = max(10, np.ceil(mx / 10) * 10)
        return int(low), int(high)

    def account_trend_chart(account_name, cid):
        trend = account_monthly.get(account_name, pd.DataFrame())
        if trend.empty:
            return ""

        labels = trend["month"].tolist()
        values = [0 if pd.isna(v) else round(v, 0) for v in trend["value"]]
        monthly_gains = [0 if pd.isna(v) else round(v, 2) for v in trend["monthly_gain_pct"]]
        cumulative_gains = [0 if pd.isna(v) else round(v, 2) for v in trend["cumulative_gain_pct"]]
        value_max = nice_axis_max(trend["value"], 10000000)
        gain_min, gain_max = nice_pct_bounds(pd.Series(monthly_gains + cumulative_gains))
        datasets = [
            {
                "type": "line",
                "label": "Value",
        ...
        return """
        <div class="chart-container" style="height:420px;margin-top:8px">
        ...
        new Chart(document.getElementById("{cid}"), {{
            ...
            options: {{
                ...
                interaction:{{mode:'index',intersect:false}},
                layout:{{padding:{{top:18,bottom:12,left:6,right:10}}}},
                elements:{{bar:{{borderRadius:0,borderSkipped:false}},line:{{tension:0,borderWidth:3}},point:{{radius:3,hoverRadius:5,hitRadius:12}}}},
                plugins:{{
                    legend:{{position:'top',align:'start',labels:{{usePointStyle:true,boxWidth:10,color:'#667085',font:{{size:12,weight:'400'}},padding:16}}}},
                    tooltip:{{
                        ...
                        callbacks:{{
                            label:function(ctx){{
                                if(ctx.dataset.yAxisID === 'y') return ctx.dataset.label+': '+Math.round(ctx.parsed.y).toLocaleString()+' ¥';
                ...
                scales:{{
                    x:{{offset:true,grid:{{display:false,drawBorder:false}},border:{{display:false}},ticks:{{autoSkip:false,maxRotation:0,minRotation:0,color:'#98a2b3',font:{{size:11,weight:'400'}}}}}},
                    y:{{type:'linear',position:'left',min:0,max:{value_max},ticks:{{stepSize:{value_step},color:'#98a2b3',font:{{size:11,weight:'400'}},callback:function(v){{return (v/1000000).toFixed(0)+'M';}}}},title:{{display:true,text:'Value ¥',color:'#667085',font:{{size:11,weight:'400'}}}},grid:{{display:true,color:'#edf2f7',lineWidth:1}},border:{{display:false}}}},
                    y1:{{type:'linear',position:'right',min:{gain_min},max:{gain_max},ticks:{{stepSize:10,color:'#98a2b3',font:{{size:11,weight:'400'}},callback:function(v){{return (v>0?'+':'')+v.toFixed(0)+'%';}}}},title:{{display:true,text:'Gain %',color:'#667085',font:{{size:11,weight:'400'}}}},grid:{{drawOnChartArea:false}},border:{{display:false}}}}
                }}
            }}
        }});
        </script>
        """.format(
            ...
            value_max=value_max,
            value_step=max(int(value_max / 5), 1000000),
            gain_min=gain_min,
            gain_max=gain_max,
        )

    def account_performance_html(performance, title, account_name, cid):
        ...
        {account_trend_chart(account_name, cid)}
        </div>
        """

    html += account_performance_html(ideco, "iDeCo Performance", "iDeCo", "perf_ideco")
    html += account_performance_html(nisa, "NISA Performance", "NISA", "perf_nisa")
    html += account_performance_html(taxable, "特定口座 Performance", "特定", "perf_taxable")
    html += "</details>"

    html += """
    <details class="private-section"><summary><span class="section-title">Investment Rules (Private)</span></summary>
    <div class="card">
    <h2 class="section-title">Investment Rules</h2>
    <ol class="rule-list">
        <li>NISAは年間投資枠を基本的に使い切る。主軸はQQQとし、SOXは市場環境・半導体セクターの過熱感・下落余地を見て補助的に使う。</li>
        <li>特定口座では一度に大きく入れず、原則50万円単位で段階投入する。特定口座の投資額が所定の金額に達したら、NISA年間枠の残額を補填する形でNISA側へ優先配分する。</li>
        <li>大幅下落時の追加投資に備えて、50万から100万円の待機資金を現金口座とは別枠で確保する。通常の生活資金と投資待機資金は混同しない。</li>
        <li>現金の追加投入は、待機資金が100万円以上に積み上がった時、または市場に大幅下落が発生した時を基本条件とする。投入先は、リスクを許容できる範囲でNASDAQ100 3x Bullなど高い反発ゲインを期待できる商品を優先候補にする。</li>
        <li>NISAは長期保有前提の非課税枠として扱い、短期の暴落では売却しない。下落時も保有を継続できる金額だけをNISAへ入れる。</li>
        <li>追加投資後は、購入タイミング別のゲイン、口座別の資産額、月別ゲイン、累積ゲインを確認し、入金による資産増と運用成果を分けて判断する。</li>
    </ol>
    <p class="rule-note">このルールは資産形成の行動基準として使う。市場環境が大きく変わった場合でも、NISA長期保有・待機資金確保・段階投入の原則を先に確認してから変更する。</p>
    </div>
    </details>
    """
...
def main():
    ...
    parser.add_argument(
        "--skip-backfill",
        action="store_true",
        help="Skip automatic missing-date history sync before the latest fetch.",
    )
    args = parser.parse_args()

    if not args.build_only and not args.skip_backfill:
        print("\n=== BACKFILL HISTORY ===")
        automatic_backfill = args.backfill is None
        backfill_histories(
            args.backfill if args.backfill is not None else ["all"],
            continue_on_error=automatic_backfill,
        )
